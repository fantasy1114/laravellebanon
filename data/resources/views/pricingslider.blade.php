...
<body class="vertical-layout vertical-menu-modern  navbar-floating footer-static  " data-open="click"
    ...
    <!-- END LAYOUT -->
    <!-- BEGIN: Content-->
    <div class="app-content content @if(Auth::user()->rolefunction->pricing_view != 'on') data-page-close @endif">
                            ...
                            <table class="user-list-table table">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th>photo</th>
                                        <th>text</th>                
                                        <th class="@if(Auth::user()->rolefunction->pricing_delete != 'on' && Auth::user()->rolefunction->pricing_write != 'on') data-page-close @endif">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $index = 1; ?>
                                    @foreach ($pricingsliders as $pricingslider)
                                        <tr>
                                            ...
                                            <td>{{$pricingslider->title}}</td>
                                            <td><img src="{{$pricingslider->photo}}" style="width:48px; height:34px;"></td>
                                            <td>
                                                {{$pricingslider->description}}
                                            </td>
                                            <td>
                                                <button class="dropdown-item userupdate_new @if(Auth::user()->rolefunction->pricing_write != 'on') data-page-close @endif"
                                                    data-id="{{$pricingslider->id}}" data-title="{{$pricingslider->title}}"
                                                    data-photo="{{$pricingslider->photo}}" data-description="{{$pricingslider->description}}" @if(Auth::user()->status == '') disabled @endif
                                                    >
                                                ...
                                                <button class="dropdown-item delete_pricingsloder_data @if(Auth::user()->rolefunction->pricing_delete != 'on') data-page-close @endif" data-id="{{$pricingslider->id}}" @if(Auth::user()->status == '') disabled @endif>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round"
    ...
    @if(Auth::user()->rolefunction->pricing_create != "on")
        <script>
